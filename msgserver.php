...
function registerpass($username,$password)
{
    ...
    if ($dbconnect->errno != 0){
        ...
        echo "Successfully connected to dat=base".PHP_EOL;
    }
...
function request_processor($request){
	...
	switch($type){
        ...
        case "populate":
            return populatepass();
        case "apipush":
            return apipass($request['uscases'], $request['usdeaths'], $request['njcases'], $request['njdeaths'], $request['nycases'], $request['nydeaths']);
    }
